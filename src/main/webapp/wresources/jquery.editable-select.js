...
(function($) {
	...
	EditableSelect.prototype = {
		...
		init : function(select, settings) {
			...
			this.text.data('editable-selecter', this.select
					.data('editable-selecter'));
		...
		checkScroll : function() {
			...
			if (options.length > this.settings.items_then_scroll) {
				this.list_height = this.list_item_height
						* this.settings.items_then_scroll;
		...
		removeAllOption:function(){
			this.wrapper.find('ul li').remove();
		},
		initInputEvents : function(text) {
			...
			text.focus(function() {
				// Can't use the blur event to hide the list, because the blur
				// event
			...
			function(e) {
				...
				timer = setTimeout(function() {
					// If the user types in a value, select it if it's in the
					// list
			...
			list_items.each(function() {
				if (!value_found) {
					...
					if (text == current_value) {
						...
						return false;
					} else if (text.indexOf(current_value) === 0
							&& !best_candiate) {
						// Can't do return false here, since we still need to
						// iterate over
		...
		scrollToListItem : function(list_item) {
			if (this.list_height) {
				this.wrapper.scrollTop(list_item[0].offsetTop
						- (this.list_height / 2));
			}
			;
		},
		...
		setWidths : function() {
			// The text input has a right margin because of the background arrow
			// image
			// so we need to remove that from the width
			var width = this.select.width() + 2;
			var padding_right = parseInt(this.text.css('padding-right')
					.replace(/px/, ''), 10);
